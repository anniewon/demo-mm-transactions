...
<grammar xmlns="http://www.w3.org/2001/06/grammar" version="1.0"
  ...
  <rule id="rootrule" scope="public">
    <one-of>
      ...
      %s
      <item><ruleref uri="#chat"/><tag>out.action = "chat"</tag></item>
    </one-of>
  </rule>
  ...
  <rule id="moreThanAmount">
    <item>
      ...
      than <ruleref uri="#number"/><tag>out = rules.number;</tag>
  ...
  <rule id="lessThanAmount">
    <item>
      less than <ruleref uri="#number"/><tag>out = rules.number;</tag>
      <item repeat="0-1">dollars</item>
    </item>
  </rule>
...
use constant MAINMENU_ROOT_RULES => << 'End';
      ...
      <item>contact us<tag>out.action = "contact"</tag></item>
      <item><ruleref uri="#report"/><tag>out.action = "report missing"</tag></item>
End
